// ============================================
//  PROHIBIDO redistribuir este código sin autorizacion expresa
// el codigo de filtros de privados fue adaptado y modificado por https://github.com/ulises12354
// autor original FELIX-OFC https://github.com/FELIX-OFC
// ============================================
import fs from 'fs';
...
export async function handler(chatUpdate) {
  ...
  this.uptime = this.uptime || Date.now();                                                                                                        if (!chatUpdate) return;
  ...
  try {
    ...
    if (!m) return;
    if (!m.isGroup) {
  const text = (m.text || '').trim().toLowerCase()                                                                                                const comandos = ['#code', '#codeprem', '#qr', '#tokens', '#addtoken', '#deltoken']
  if (!comandos.includes(text)) return
}
    m.exp = 0;
    m.coin = false;
