...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
    ...
    if (new Date() - global.db.data.users[m.sender].Subs < 120000) {
        return conn.reply(m.chat, `🕐 Debes esperar ${msToTime(time - new Date())} para volver a vincular un *prem bot.*`, m);
    }
    const subBots = [...new Set([...global.conns.filter((conn) => conn.user && conn.ws.socket && conn.ws.socket.readyState !== ws.CLOSED).map((conn) => conn)])];
    const subBotsCount = subBots.length;
    if (subBotsCount === 30) {
        return conn.reply(`❀ No se han encontrado servidores para *Sub-Bots* disponibles.`);
    }
    ...
    blackJBOptions.isPremiumFromToken = validToken.premium; // Pasa el estado premium del token
...
export async function blackJadiBot(options) {
    ...
    exec(checkSecurity, async (err, stdout, stderr) => {
        ...
        async function connectionUpdate(update) {
            ...
            if (connection == `open`) {
                ...
                const premiumConfig = { premiumBot: options.isPremiumFromToken };
