...
conn.reply(m.chat, `☆ Espera *${tiempo2}* para volver a Trabajar.`, m, fake)
return
}
let rsl = Math.floor(Math.random() * 5000)
cooldowns[m.sender] = Date.now()
await conn.reply(m.chat, `${pickRandom(trabajo)} *${toNum(rsl)}* ( *${rsl}* ) XP.`, m, fake)
user.exp += rsl
}
...
// Thanks to FG98
const trabajo = [
   "✿ Cumpliste la mayoría de edad y tus padres te consiguieron trabajo justo el día despues de cumplirlos, ¡Bienvenido a la sociedad!, Ganas",
   "✿ Trabajas para una empresa militar privada, y ganas",
